...
class ERMD_Strategy_amount_4h_422(IStrategy):
    ...
    stoploss = -0.329  # maximum stop loss distance
    ...
    buy_total_risk_trade = DecimalParameter(0.007, 0.025, decimals=3, default=0.018, optimize=True)
    buy_cust_stoploss_ = DecimalParameter(0.1, 0.3, decimals=2, default=0.1, optimize=True)
    # Parameters: signal
    buy_ema_enabled = BooleanParameter(default=True, optimize=True)
    buy_ema1_length = IntParameter(3, 100, default=12, optimize=True)
    buy_ema2_length = IntParameter(3, 100, default=65, optimize=True)
    buy_ema3_length = IntParameter(3, 100, default=96, optimize=True)
    buy_rsi_enabled = BooleanParameter(default=False, optimize=True)
    buy_rsi_length = IntParameter(3, 100, default=12, optimize=True)
    buy_rsi_threshold = IntParameter(50, 100, default=61, optimize=True)
    buy_macd_enabled = BooleanParameter(default=True, optimize=True)
    buy_macd_fast = IntParameter(3, 50, default=38, optimize=True)
    buy_macd_slow = IntParameter(20, 200, default=91, optimize=True)
    buy_macd_smooth = IntParameter(1, 30, default=22, optimize=True)
    ...
    sell_ema1_length = IntParameter(3, 100, default=17, optimize=True)
    sell_ema2_length = IntParameter(3, 100, default=94, optimize=True)
    sell_ema3_length = IntParameter(3, 100, default=40, optimize=True)
    sell_rsi_enabled = BooleanParameter(default=False, optimize=True)
    sell_rsi_length = IntParameter(3, 100, default=36, optimize=True)
    sell_rsi_threshold = IntParameter(0, 50, default=9, optimize=True)
    sell_macd_enabled = BooleanParameter(default=False, optimize=True)
    sell_macd_fast = IntParameter(3, 50, default=49, optimize=True)
    sell_macd_slow = IntParameter(20, 200, default=28, optimize=True)
    sell_macd_smooth = IntParameter(1, 30, default=5, optimize=True)
    ...
    sell_high_line = IntParameter(-25, 0, default=-19, space="sell", optimize=True)
    sell_low_line = IntParameter(-100, -75, default=-80, space="sell", optimize=True)
    sell_ema = IntParameter(7, 22, default=8, space='sell', optimize=True)
    sell_willy = IntParameter(15, 30, default=15, space='sell', optimize=True)
    buy_sl_mult = DecimalParameter(0.5, 5, default=3.7, decimals=1, space='buy', optimize=True)
    ...
    def custom_stake_amount(self, pair: str, current_time: datetime, current_rate: float,
        ...
        stake_em = self.wallets.get_total_stake_amount() * self.buy_total_risk_trade.value

        if Debug_Amount:
            print(stake_em, (self.buy_cust_stoploss_.value * 100))
            print("Stake amount", stake_em / self.buy_cust_stoploss_.value)
        return stake_em / self.buy_cust_stoploss_.value
