...
class Heikin_Ashi1D51_will_edge(IStrategy):
    INTERFACE_VERSION = 3
    ...
    buy_RSI_short = IntParameter(30, 60, default=45, space="buy")
    buy_cust_one_trade_limit_max = IntParameter(5, 30, default=24, space="buy")
    buy_cust_one_trade_limit_min = IntParameter(1, 8, default=3, space="buy")
    buy_low_line = IntParameter(-100, -70, default=-93, space="buy")
    buy_high_line = IntParameter(-30, 0, default=-18, space="buy")
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe['willy'] = self.willy_ema(dataframe, low_line=self.buy_low_line.value, up_line=self.buy_high_line.value, willyLen=21, emaLen=13)['Willy']
        dataframe['ema_will'] = self.willy_ema(dataframe, low_line=-80, up_line=-20, willyLen=21, emaLen=13)['Ema_will']
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[
            (
                    ...
                    (dataframe['willy'] < self.buy_high_line.value) &
                    (dataframe['ema_will'] < self.buy_high_line.value)
            ),
            'enter_long'] = 1
        ...
        dataframe.loc[
            (
                    ...
                    (dataframe['willy'] > self.buy_low_line.value) &
                    (dataframe['ema_will'] > self.buy_low_line.value)
            ),
            'enter_short'] = 1
    ...
    def willy_ema(self, dataframe: DataFrame, low_line=-80.0, up_line=-20.0, willyLen=21, emaLen=13):
        ...
        df['lower'] = df['highest'].rolling(willyLen).min()
