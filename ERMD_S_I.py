...
class ERMD_S_I(IStrategy):
    ...
    max_pos_pct = DecimalParameter(0.01, 0.5, default=0.12, decimals=2, optimize=True,
                                   space="buy")  # maximum position size, percent of current portfolio value

    # stop loss
    stoploss = -0.328  # maximum stop loss distance
    tsl_break_even = DecimalParameter(0.0, 0.1, default=0.1, decimals=2, optimize=True,
                                      space="buy")  # place break-even stop loss when desired profit is reached
    tsl_start_offset = DecimalParameter(0.0, 0.1, default=0.019, decimals=3, optimize=True,
                                        space="buy")  # start trailing stop loss when this profit level is reached
    tsl_trailing_distance = DecimalParameter(0.01, 0.2, default=0.07, decimals=2, optimize=True,
                                             space="buy")  # distance to price
