...
class ERMD_Strategy_leveraged(IStrategy):
    ...
    max_pos_pct = DecimalParameter(0.01, 0.5, default=0.12, decimals=2, optimize=True, space="buy")         # maximum position size, percent of current portfolio value

    # stop loss
    stoploss = -0.328  # maximum stop loss distance
    tsl_break_even = DecimalParameter(0.0, 0.1, default=0.1, decimals=2, optimize=True, space="buy")       # place break-even stop loss when desired profit is reached
    tsl_start_offset = DecimalParameter(0.0, 0.1, default=0.019, decimals=3, optimize=True, space="buy")    # start trailing stop loss when this profit level is reached
    tsl_trailing_distance = DecimalParameter(0.01, 0.2, default=0.07, decimals=2, optimize=True, space="buy")      # distance to price
    ...
    buy_ema1_length = IntParameter(3, 100, default=24, optimize=True)
    buy_ema2_length = IntParameter(3, 100, default=12, optimize=True)
    buy_ema3_length = IntParameter(3, 100, default=34, optimize=True)
    buy_rsi_enabled = BooleanParameter(default=False, optimize=True)
    buy_rsi_length = IntParameter(3, 100, default=12, optimize=True)
    buy_rsi_threshold = IntParameter(50, 100, default=55, optimize=True)
    buy_macd_enabled = BooleanParameter(default=False, optimize=True)
    buy_macd_fast = IntParameter(3, 50, default=41, optimize=True)
    buy_macd_slow = IntParameter(20, 200, default=74, optimize=True)
    buy_macd_smooth = IntParameter(1, 30, default=5, optimize=True)
    ...
    sell_ema1_length = IntParameter(3, 100, default=74, optimize=True)
    sell_ema2_length = IntParameter(3, 100, default=26, optimize=True)
    ...
    sell_rsi_length = IntParameter(3, 100, default=53, optimize=True)
    sell_rsi_threshold = IntParameter(0, 50, default=27, optimize=True)
    sell_macd_enabled = BooleanParameter(default=True, optimize=True)
    sell_macd_fast = IntParameter(3, 50, default=42, optimize=True)
    sell_macd_slow = IntParameter(20, 200, default=67, optimize=True)
    sell_macd_smooth = IntParameter(1, 30, default=13, optimize=True)
    sell_candlestick_enabled = BooleanParameter(default=True, optimize=True)

    sell_willy_exit = BooleanParameter(default=True, space='sell', optimize=True)
    sell_high_line = IntParameter(-25, 0, default=-14, space="sell", optimize=True)
    sell_low_line = IntParameter(-100, -75, default=-94, space="sell", optimize=True)
    sell_ema = IntParameter(7, 22, default=15, space='sell', optimize=True)
